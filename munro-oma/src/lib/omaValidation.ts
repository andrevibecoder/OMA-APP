...
// a half-done OMA). An OMA that saved without a metric shows as 0% / "No metric
// set yet", which reads to the owner as "it didn't save".

type MetricShape = { measure: string; target: number }

export function omaSaveBlockers(input: { outcome: string; metrics: MetricShape[] }): string[] {
